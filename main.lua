...
rem.Parent = script
rem.Name = "Remote"
local remf = Instance.new("RemoteFunction")
remf.Parent = script
remf.Name = "RemoteF"
local key = genguid()
local ls = NLS([[
...
local remf = script.Parent:WaitForChild'RemoteF'
local keyt = script:WaitForChild'Instance'.ServerInstanceId
local mouse = owner:GetMouse()
local uis = game:GetService("UserInputService")
rem:FireServer()
local snds = {}
remf.OnClientInvoke = function(v)
print(v)
	snds[v] = 1
...
fakeInstance = {new = function(class,parent)
	if class == 'Sound' then
		local snd = Instance.new(class)
		snd.Parent = parent or realowner.Character
		remf:InvokeClient(realowner,snd)
		snd = wrap(snd)
		sndlist[snd] = 0
...
rem.OnServerEvent:Connect(function(who,akey,type,...)
	if akey == key then
		...
		if type == 'setmouse' then
			...
			mouse.X = args.X
			mouse.Y = args.Y
			mouse.Target = args.Target
			mouse.Origin = args.Origin
			mouse.TargetSurface = args.TargetSurface
			mouse.ViewSizeX = args.ViewSizeX
			mouse.ViewSizeY = args.ViewSizeY
...
local snd = fakeInstance.new("Sound",owner.Character.Head)
snd.Volume = 4
snd.SoundId = "rbxassetid://516046413"
snd:Play()
wait(1)
print(snd.PlaybackLoudness)
